...
N <- 100000
tibble(x = c(rnorm(N, sd = 1.5),
             rnorm(N, sd = 4),
             sn::rsn(N, alpha = -4, omega = 4, xi = 3)),
       y = c(rep("a", N), rep("b", N), rep("c", N))) |> 
  ggplot(aes(x = x)) +
...
```{r}
#| eval: false
p1 <- tibble(race = rep(c("AAPI", "Black", "Hispanic", "Native\nAmerican", "White"), 3),
       probs = round(
         c(MCMCpack::rdirichlet(1, c(1, 1, 20, 1, 1)),
           MCMCpack::rdirichlet(1, c(1, 1, 10, 1, 10)),
           MCMCpack::rdirichlet(1, c(5, 5, 8, 5, 5))), 
         3),
       model = c(rep("a", 5),
                 rep("b", 5),
                 rep("c", 5))) |> 
  ggplot(aes(x = probs, y = model, fill = race)) +
...
p2 <- ggplot() +
  stat_function(fun = dnorm,
                args = list(sd = 1), 
                geom = "area", fill = met.brewer("Cross")[[7]], alpha = 1) +
    stat_function(fun = dnorm,
                args = list(sd = 1), 
                color = "white", linewidth = 1, linetype = "solid") +
  xlim(-9, 9) +
  theme_void()
...
p4 <- ggplot() +
  stat_function(fun = sn::dsn,
                args = list(alpha = -3, omega = 3, xi = 3), 
                geom = "area", fill = met.brewer("Cross")[[7]], alpha = 1) +
  stat_function(fun = sn::dsn,
                args = list(alpha = -3, omega = 3, xi = 3), 
                color = "white", linewidth = 1, linetype = "solid") +
